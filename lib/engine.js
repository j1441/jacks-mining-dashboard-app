...
const STATUS_TEMPLATES = {
  ...
  'off-unprofitable': {
    severity: 'ok',
    render: (p) => `Off — mining unprofitable (price ${fmt2(p.price)} kr/kWh) and no heat needed.`
...
function decide(inputs) {
  ...
  // ---- 3) tuning hold ----------------------------------------------------------
  if (s.tunerState === 'TUNING' || s.tunerState === 'PREHEAT') {
    if (!state.tuningHoldSince) stateUpdates.tuningHoldSince = now;
    blockedBy.push('tuner tuning');
    return finish(null, 'tuner-tuning', {}, curChosen);
  }
  if (state.tuningHoldSince) stateUpdates.tuningHoldSince = null;
  ...
  const offStatusParams = () => {
    ...
    return { price, nextStartHour: next && next.hour, nextStartPrice: next && next.price };
  };
  ...
  if (emergencyEligible) {
    ...
    if (pick.boards !== s.boardsOn) {
      ...
      else pick = sameEligible[0] || (sameBest ? onCands.filter((c) => c.boards === s.boardsOn).reduce((a, b) => (b.wallW > a.wallW ? b : a)) : pick);
    }
  } else {
    pick = sameBest;
