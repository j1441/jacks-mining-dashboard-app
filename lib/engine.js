...
const STATUS_TEMPLATES = {
  ...
  'comfort-limit': {
    severity: 'ok',
    render: (p) => `Paused — room ${fmt1(p.roomC)} °C is at the ${fmt1(p.maxC)} °C comfort limit; profit mining resumes once it cools.`,
  },
  'off-unprofitable': {
    severity: 'ok',
...
// scoreCandidate(c, {marginalPrice, hashpriceNokPerThDay, poolFeePct, btcPremiumPct, heat}) -> ScoredCandidate
// btcPremiumPct values mined sats above exchange spot (e.g. 6 = the buyer would
// pay 6% over spot for non-KYC coins, so mined revenue is worth 6% more).
function scoreCandidate(c, { marginalPrice, hashpriceNokPerThDay, poolFeePct = 0, btcPremiumPct = 0, heat }) {
  const demandKW = heat ? (Number(heat.demandKW) || 0) : 0;
  const altPrice = heat ? (Number(heat.altPricePerKWh) || 0) : 0;
  const premium = 1 + (Number(btcPremiumPct) || 0) / 100;
  const revenueNokH = (c.hashrateThs * (hashpriceNokPerThDay || 0) * (1 - poolFeePct) * premium) / 24;
...
function scoreAll(candidates, price, ctx) {
  const args = { marginalPrice: price, hashpriceNokPerThDay: ctx.hashpriceNokPerThDay, poolFeePct: ctx.poolFeePct, btcPremiumPct: ctx.btcPremiumPct, heat: ctx.heat };
...
function boardSwitchCost({ diffCand, incumbentScore, currentPrice, econ, ctx }) {
  const retune = scoreCandidate({ ...diffCand, hashrateThs: 0 }, {
    ...
    btcPremiumPct: ctx.btcPremiumPct,
    heat: ctx.heat,
  }).scoreNokH;
...
function describeAction(a) {
  ...
  switch (a.type) {
    ...
    case 'RESUME': {
      ...
      if (a.targetW) parts.push(`heading for ${a.targetW} W`);
      return parts.length ? `resume mining (${parts.join(', ')})` : 'resume mining';
    }
...
function decide(inputs) {
  ...
  const ctx = {
    ...
    btcPremiumPct: econ.btcPremiumPct || 0,
    heat,
  };
  ...
  // Comfort ceiling: profit mining must not cook the room. Stops at maxRoomC,
  // won't (re)start until the room is 0.5° under it; heat-demand emergencies are
  // unaffected (demand implies the room is below target, hence below the max).
  // No room reading → no comfort constraint (mining is not safety-critical here).
  const comfortMax = Number(heat.maxRoomC);
  const roomNow = Number(heat.roomTempC);
  const comfortHot = Number.isFinite(comfortMax) && Number.isFinite(roomNow) && roomNow >= comfortMax;
  const comfortWarm = Number.isFinite(comfortMax) && Number.isFinite(roomNow) && roomNow >= comfortMax - 0.5;
  ...
  if (!s.running) {
    ...
    const startAction = (pick) => {
      ...
      stateUpdates.pausedBy = null;
      if (s.paused) {
        // Reconcile the enabled board set with the pick before resuming: the
        // board config can drift while paused (a disable that reported failure
        // but persisted — seen live 2026-07-26, board 3 stayed off for hours
        // because RESUME trusted the intended set instead of the snapshot).
        const ids = desiredBoardIds(snapshot, settings, pick.boards);
        if (ids.enableIds.length || ids.disableIds.length) {
          stateUpdates.lastBoardsChangeAt = now;
          reasons.push(`board config drifted while paused — reconciling to ${pick.boards} board(s)`);
          return { type: 'RESUME', boards: pick.boards, ...ids, targetW: pick.targetW, reason: `start: ${fmt2(pick.scoreNokH)} kr/h at ${pick.boards} board(s) @ ${pick.targetW} W`, severity: 'info' };
        }
    ...
    if (startOk && offDwellOk && comfortWarm && !emergencyEligible) {
      blockedBy.push(`room ${fmt1(roomNow)} °C near comfort limit ${fmt1(comfortMax)} °C`);
      return finish(null, 'comfort-limit', { roomC: roomNow, maxC: comfortMax }, { off: true });
    }
  ...
  if ((wantStop || comfortHot) && !emergencyEligible) {
    stateUpdates.lastOffAt = now;
    stateUpdates.pausedBy = 'engine';
    if (tuningHold) reasons.push('shutdown allowed through tuning hold — nothing to wait for');
    if (comfortHot && !wantStop) {
      reasons.push(`room ${fmt1(roomNow)} °C ≥ comfort limit ${fmt1(comfortMax)} °C`);
      return finish(
        { type: 'PAUSE', reason: `stop: room at ${fmt1(roomNow)} °C comfort limit`, severity: 'info' },
        'comfort-limit', { roomC: roomNow, maxC: comfortMax }, { off: true },
      );
    }
    reasons.push(bestOn
...
function buildPlan(inputs) {
  ...
  const econ = settings.economics;
  const dwell = settings.dwell;
  const ctx = { hashpriceNokPerThDay: market.hashpriceNokPerThDay, poolFeePct: econ.poolFeePct || 0, btcPremiumPct: econ.btcPremiumPct || 0, heat };
