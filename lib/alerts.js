...
// Node's fetch (undici) throws on URLs that embed credentials (https://user:pass@host).
// Move any userinfo into an Authorization: Basic header and return a clean URL.
function splitUrlCredentials(rawUrl) {
  try {
    ...
    return { url: rawUrl, headers: {} }; // let fetch surface the malformed-URL error
  }
}
...
class Alerts {
  ...
  async _dispatch({ ruleKey, severity, title, message, minerId }) {
    ...
    if (cfg.ntfy?.url && cfg.ntfy?.topic) {
      const base = `${cfg.ntfy.url.replace(/\/+$/, '')}/${encodeURIComponent(cfg.ntfy.topic)}`;
      const { url, headers } = splitUrlCredentials(base);
      results.ntfy = await this._try('ntfy', () => this.fetch(url, {
        method: 'POST',
        headers: {
          ...headers,
          Title: title,
    ...
    if (cfg.webhook?.url) {
      const { url, headers } = splitUrlCredentials(cfg.webhook.url);
      results.webhook = await this._try('webhook', () => this.fetch(url, {
        method: 'POST',
  ...
  async evaluate(ctx) {
    ...
    if (snapshot && snapshot.online) {
      ...
      // A paused miner's enabled boards legitimately aren't hashing (false
      ...
      for (const b of snapshot.boards || []) {
        ...
        if (!tuningHold && !snapshot.paused && b.enabled && !b.hashing) {
