...
const DEFAULT_CONFIG = {
  ...
  heating: {
    ...
    thermostat: { targetC: 21, bandC: 2, maxKW: 3.5, idleOffsetC: 1.5, runningOffsetC: 0 },
      maxRoomC: 27,
    ...
    // tempSensor: an external ambient sensor (Tasmota over HTTP) whose reading
    // supersedes that inlet estimate for the zone while it is fresh; type
    // 'none' keeps the inlet-derived behaviour.
    zones: [{
      id: 'home', name: 'Home',
      demandSource: 'off', manualKW: 0, schedule: null,
      presets: [{ name: 'Off', kw: 0 }, { name: 'Eco', kw: 1.0 }, { name: 'Comfort', kw: 2.5 }],
      ...
      maxRoomC: 27,
      alt: { type: 'heatpump', scop: 3.0 },
      tempSensor: { type: 'none', host: '', name: '' },
    }],
  },
  economics: {
    poolFeePct: 0, startMarginNokH: 0.5, keepMarginNokH: 0.2,
    // Mined sats valued above exchange spot (non-KYC premium, no purchase fees):
    // revenue is multiplied by (1 + btcPremiumPct/100) in the objective.
    btcPremiumPct: 0,
...
function validateConfig(cfg) {
  ...
  const checkArea = (a, p) => {
    ...
    if (a.thermostat.runningOffsetC !== undefined) {
      checkNum(a.thermostat.runningOffsetC, `${p}.thermostat.runningOffsetC`, 0, 10);
    }
    if (a.maxRoomC !== undefined) checkNum(a.maxRoomC, `${p}.maxRoomC`, 10, 40);
    // External ambient sensor. Optional: absent means the inlet-derived estimate.
    if (a.tempSensor !== undefined && a.tempSensor !== null) {
      if (!isPlainObject(a.tempSensor)) fail(`${p}.tempSensor must be an object`);
      checkEnum(a.tempSensor.type, `${p}.tempSensor.type`, ['none', 'tasmota']);
      if (a.tempSensor.type === 'tasmota') {
        checkStr(a.tempSensor.host, `${p}.tempSensor.host`, { nonEmpty: true });
      }
      if (a.tempSensor.name !== undefined) checkStr(a.tempSensor.name, `${p}.tempSensor.name`);
    }
  ...
  checkNum(ec.btcPremiumPct, 'economics.btcPremiumPct', 0, 100);
