...
const DEFAULT_CONFIG = {
  ...
  heating: {
    ...
    // Thermostat mode: demand ramps 0→maxKW as the room falls below targetC over
    // bandC degrees (modulating proportional band). Room temp comes from the miner
    // hashboards' inlet sensors; idleOffsetC corrects the reading when fans are off.
...
function validateConfig(cfg) {
  ...
  checkEnum(he.demandSource, 'heating.demandSource', ['off', 'manual', 'schedule', 'thermostat']);
  checkNum(he.manualKW, 'heating.manualKW', 0, 100);
  if (!isPlainObject(he.thermostat)) fail('heating.thermostat must be an object');
  checkNum(he.thermostat.targetC, 'heating.thermostat.targetC', 5, 35);
  checkNum(he.thermostat.bandC, 'heating.thermostat.bandC', 0.5, 10);
  checkNum(he.thermostat.maxKW, 'heating.thermostat.maxKW', 0, 100);
  checkNum(he.thermostat.idleOffsetC, 'heating.thermostat.idleOffsetC', 0, 10);
  if (he.schedule !== null) {
