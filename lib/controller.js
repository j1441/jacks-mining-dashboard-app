...
class Controller {
  ...
  async buildInputs(cfg, minerCfg, snapshot) {
    ...
    const ownRoomC = estimateRoomTempC(snapshot, zone.thermostat);
    this.roomTempC = ownRoomC; // cached for snapshotForApi
  ...
  currentEconomics(snapshot, cfg) {
    ...
    const zone = zoneForMiner(cfg, this.id);
    const tz = ((cfg.electricity || {}).timezone) || 'Europe/Oslo';
    const roomC = zoneRoomTemp(this.roomTemps, zone.id, estimateRoomTempC(s, zone.thermostat));
    const heat = {
  ...
  snapshotForApi() {
    ...
    return {
      ...
      roomTempC: estimateRoomTempC(s, (zoneForMiner(cfg, this.id).thermostat)),
...
module.exports = {
  Controller, resolveHeatDemand, altHeatPrice, estimateRoomTempC,
