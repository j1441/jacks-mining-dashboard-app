...
// Which per-board sensor stands in for ambient.
//
// lowest_inlet_temp is the air being pulled into the board, so it is the right
// sensor — but Braiins only publishes it for a board that is actually hashing.
// Measured live 2026-07-29: the paused garage S19j Pro returned board_temp and
// chip_temp on all three boards and inlet/outlet null on every one, while the
// hashing S19J4 next door returned inlet 35 °C on its one live board and all
// nulls on its two disabled boards. Keying room temperature off the inlet alone
// therefore left every paused miner with no reading at all — a zone whose miner
// was parked (which, outside the heating season, is most of the time) showed
// "–" and its thermostat sat at 0 kW demand.
//
// A stopped board's PCB is the usable stand-in: with no hashing load and the
// idle fans still moving room air through the chassis it settles to ambient
// plus a degree or two (live: garage PCB 24.0/24.5/24.0 °C with chips at 39 °C).
// While hashing it is not a stand-in for anything — the same day the hashing
// S19J4 read PCB 47.5 °C against an inlet of 35 °C — so the fallback is gated on
// the miner being stopped.
function ambientProxyTempsC(snapshot, hashing) {
  return ((snapshot || {}).boards || [])
    .map((b) => {
      if (!b) return null;
      if (b.inletTempC != null) return Number(b.inletTempC);
      if (!hashing && b.boardTempC != null) return Number(b.boardTempC);
      return null;
    })
    .filter((n) => n !== null && Number.isFinite(n));
}

const isHashing = (snapshot) => ((snapshot || {}).boardsHashingCount || 0) > 0;

// Room temperature estimate from the miner's own sensors: the minimum ambient
// proxy across the hashboards (see ambientProxyTempsC) ≈ the air entering the
// chassis. A stopped miner is reading a sensor sitting inside a chassis that
// still holds standby heat, so subtract idleOffsetC. The offset keys off hashing
// rather than fan speed so this agrees with classifyRoomReading — the fans keep
// turning when mining stops, so "fans on" does not mean "reading is clean".
// Returns °C rounded to 0.1, or null when no board offers a usable sensor.
function estimateRoomTempC(snapshot, thermo) {
  if (!snapshot || !snapshot.online) return null;
  const hashing = isHashing(snapshot);
  const proxies = ambientProxyTempsC(snapshot, hashing);
  if (!proxies.length) return null;
  const off = Number((thermo || {}).idleOffsetC);
  const t = Math.min(...proxies) - (hashing ? 0 : (Number.isFinite(off) ? off : 1.5));
...
// Reading reliability: the ambient proxy tracks the room only when the fans are
// pulling room air at speed while hashing, or when a stopped miner has settled.
...
// steadyMinutes: how long fans have been continuously ≥2000 rpm while hashing.
// A brief fan spin-up over a hot chassis still reads the trapped heat (seen
// live: a startup fan burst captured "26.0° room" vs 21.4° actual) — the
// running reading only counts after the airflow has flushed the chassis.
// idleMinutes: minutes since the miner last stopped (null = unknown/long ago).
// Idle readings are only trusted after a real cooldown — chips pass 45° long
// before the enclosure air settles (seen live: chips 45° with inlet still 36°,
// which minus the idle offset poisoned the estimate at "29.5° room").
// runningOffsetC compensates exhaust recirculation into the intake while the
// fans are at speed (seen live: reading crossed the target within 7 min of a
// heating start while a thermometer showed the room unchanged at 19.9°).
//
// Braiins also does not park the fans when mining stops, so "the fans are off"
// was never a usable proxy for "the chassis has cooled": measured live on
// 2026-07-29 with both miners at zero hashrate, the garage S19j Pro idled at
// ~1000 rpm (fan speed 0 %) and the S19J4 sat at 3000 rpm. The idle branch below
// used to require fanMax < 100, which neither miner ever satisfies, so even once
// a paused miner has a usable sensor its reading was still classified
// unreliable. Fans still turning is in fact the easier case — a stopped chassis
// under airflow settles to ambient far sooner than one cooling passively — so it
// gets the shorter dwell. The chip ceiling, not the fan speed, is what rejects a
// just-paused hot chassis.
const FANS_STEADY_MIN = 10; // minutes of sustained airflow before running readings count
const IDLE_FLUSHED_MIN = 15; // minutes stopped with the fans still moving air
const IDLE_COOLED_MIN = 45;  // minutes stopped with the fans off — passive cooldown is slow
const IDLE_CHIP_MAX_C = 45;  // above this the boards are still shedding heat into the intake

function classifyRoomReading(snapshot, thermo, idleMinutes, steadyMinutes) {
  if (!snapshot || !snapshot.online) return null;
  const hashing = (snapshot.boardsHashingCount || 0) > 0;
  const proxies = ambientProxyTempsC(snapshot, hashing);
  if (!proxies.length) return null;
  const minAmbient = Math.min(...proxies);
  if (minAmbient < -30 || minAmbient > 60) return null;
  const fans = ((snapshot.cooling && snapshot.cooling.fans) || []).map((f) => Number(f.rpm) || 0);
  const fanMax = fans.length ? Math.max(...fans) : 0;
  const chips = (snapshot.boards || [])
    .filter((b) => b && b.chipTempC != null)
    .map((b) => Number(b.chipTempC))
    .filter(Number.isFinite);
  const chipMax = chips.length ? Math.max(...chips) : null;
  const num = (v, d) => (Number.isFinite(Number(v)) ? Number(v) : d);
  const idleOff = num((thermo || {}).idleOffsetC, 1.5);
  const runOff = num((thermo || {}).runningOffsetC, 0);
  const r1v = (n) => Math.round(n * 10) / 10;
  if (hashing && fanMax >= 2000) {
    const flushed = Number(steadyMinutes) >= FANS_STEADY_MIN;
    return { tempC: r1v(minAmbient - runOff), reliable: flushed };
  }
  if (!hashing) {
    // Missing chip temps mean the boards are (re)initialising, not cooled —
    // a mid-restart snapshot once passed this gate and captured a hot-chassis
    // reading as "28.0° room". Unknown ≠ cool.
    const dwellMin = fanMax >= 100 ? IDLE_FLUSHED_MIN : IDLE_COOLED_MIN;
    const cooled = (chipMax !== null && chipMax <= IDLE_CHIP_MAX_C)
      && (idleMinutes === null || idleMinutes === undefined || idleMinutes >= dwellMin);
    return { tempC: r1v(minAmbient - idleOff), reliable: cooled };
  }
  return { tempC: r1v(minAmbient), reliable: false }; // transitional state
}
...
//
// sensorTempC, when a zone has an external ambient sensor with a fresh reading,
// supersedes the miner-derived estimates outright — it is not min'd with them.
// The inlet estimate is a proxy for ambient that the miner's own heat biases
// upward in the windows classifyRoomReading rejects; a thermometer in the room
// is simply the better measurement, and mixing the two would let a stale-warm
// or spuriously-cold inlet value override the real one.
function zoneRoomTemp(roomTemps, zoneId, ownTempC, sensorTempC = null) {
  if (Number.isFinite(sensorTempC)) return sensorTempC;
  const temps = [];
...
function describeAction(action) {
  switch (action.type) {
    ...
    case 'RESUME': {
      ...
      if (en || dis) return `resume mining (reconcile boards: enable [${en}], disable [${dis}])`;
...
// --- Controller ---------------------------------------------------------------
class Controller {
  constructor({ minerCfg, client, envelope, market, engine, stateStore, history, alerts, wsHub, configStore, roomTemps, tempSensors }) {
    this.roomTemps = roomTemps || null; // shared Map minerId -> {zoneId, tempC, ts}
    this.tempSensors = tempSensors || null; // TempSensors; null = inlet estimate only
  ...
  async tick(preloadedSnapshot) {
    ...
    try {
      ...
      await this.maybeApplyCooling(snapshot);
  ...
  async buildInputs(cfg, minerCfg, snapshot) {
    ...
    const offAt = parseTs(this.state.lastOffAt);
    const idleMinutes = offAt === null ? null : (Date.now() - offAt) / 60000;
    // Sustained-airflow tracker (in-memory on purpose: a restart re-qualifies).
    const fanMaxNow = Math.max(0, ...((snapshot.cooling && snapshot.cooling.fans) || []).map((f) => Number(f.rpm) || 0));
    if ((snapshot.boardsHashingCount || 0) > 0 && fanMaxNow >= 2000) {
      if (!this._fansSteadyAt) this._fansSteadyAt = Date.now();
    } else {
      this._fansSteadyAt = null;
    }
    const steadyMinutes = this._fansSteadyAt ? (Date.now() - this._fansSteadyAt) / 60000 : 0;
    const reading = classifyRoomReading(snapshot, zone.thermostat, idleMinutes, steadyMinutes);
    ...
    const sensorTempC = this.tempSensors ? this.tempSensors.tempC(zone.id) : null;
    const roomTempC = zoneRoomTemp(this.roomTemps, zone.id, ownRoomC, sensorTempC);
    this.roomTempSource = Number.isFinite(sensorTempC) ? 'sensor' : (roomTempC === null ? 'none' : 'inlet');
    ...
    const heat = {
      ...
      roomTempC,
      roomTempSource: this.roomTempSource,
      maxRoomC: Number.isFinite(Number(zone.maxRoomC)) ? Number(zone.maxRoomC) : null,
      zoneId: zone.id || null,
    };
    if (zone.demandSource === 'thermostat' && snapshot.online && roomTempC === null) {
      if (!this._thermoSensorWarned) {
        this._thermoSensorWarned = true;
        const configured = zone.tempSensor && zone.tempSensor.type === 'tasmota';
        await this.appendEvent('thermostat-sensor-unavailable', 'warn',
          configured
            ? 'thermostat active but neither the room sensor nor a hashboard inlet temperature is available — heat demand held at 0'
  ...
  async actuate(action) {
    switch (action.type) {
      ...
      case 'RESUME': {
        // Engine attaches enable/disableIds when the board config drifted while
        // paused (e.g. a disable that reported failure but persisted). Apply the
        // board set BEFORE resuming so the hashchain comes up once with the
        // intended boards instead of restarting for a board change right after.
        if ((action.enableIds && action.enableIds.length) || (action.disableIds && action.disableIds.length)) {
          await this.client.setBoards(action.enableIds || [], action.disableIds || []);
          // Boards DID change: record the dwell even if the resume below fails,
          // or the rate limiter would treat the switch as never having happened.
          this.state.lastBoardsChangeAt = nowIso();
          await this.persistState();
        }
  ...
  currentEconomics(snapshot, cfg) {
    ...
    const roomC = zoneRoomTemp(this.roomTemps, zone.id, this.roomTempC != null ? this.roomTempC : null,
      this.tempSensors ? this.tempSensors.tempC(zone.id) : null);
    const heat = {
    ...
    try {
      sc = this.engine.scoreCandidate(
        { boards: s.boardsEnabledCount, targetW: s.tuner ? s.tuner.targetW : 0, hashrateThs: hr, wallW: s.wallW },
        {
          ...
          btcPremiumPct: ((cfg.economics || {}).btcPremiumPct) || 0,
          heat,
        }
      );
  ...
  snapshotForApi() {
    ...
    return {
      ...
      ip: minerCfg.ip,
      online: !!s.online,
      // Exposed so the UI can tell a deliberate pause from a hardware fault:
      // a paused miner's boards stay enabled and stop hashing, which is
      // indistinguishable from a dead board without this flag.
      paused: !!s.paused,
      mode: minerCfg.mode,
      dryRun: !!minerCfg.dryRun,
  ...
  // Applies the configured cooling mode when cooling.manage is on and the miner
  // disagrees. Throttled to one attempt per 5 min — the config read that reports
  // the live mode is cached ~60 s, so back-to-back retries would double-apply.
  // ⚠ SetCoolingMode is SAVE_AND_APPLY: bosminer restarts and re-tunes once.
  async maybeApplyCooling(snapshot) {
    const c = this.minerCfg.cooling || {};
    if (!c.manage || !snapshot.online) return;
    const observed = snapshot.cooling && snapshot.cooling.mode;
    const desired = c.mode || 'auto';
    if (!observed || observed === 'unknown' || observed === desired) return;
    const last = this._lastCoolingApplyMs || 0;
    if (Date.now() - last < 5 * 60 * 1000) return;
    this._lastCoolingApplyMs = Date.now();
    try {
      await this.client.setCoolingMode(desired === 'auto'
        ? { mode: 'auto', targetC: (c.targetC || 60) }
        : { mode: desired, targetC: (c.targetC || 60) });
      await this.appendEvent('cooling-config', 'info',
        `cooling mode set to ${desired} (was ${observed}) — bosminer restarts and re-tunes once`);
    } catch (e) {
      await this.appendEvent('cooling-config-failed', 'warn',
        `failed to set cooling mode to ${desired}: ${e.message}`);
    }
  }
