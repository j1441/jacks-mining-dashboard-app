...
function classifyRoomReading(snapshot, thermo, idleMinutes) {
  if (!snapshot || !snapshot.online) return null;
  const inlets = (snapshot.boards || [])
    .filter((b) => b && b.inletTempC != null)
    .map((b) => Number(b.inletTempC))
  ...
  if (hashing && fanMax >= 2000) return { tempC: r1v(minInlet - runOff), reliable: true };
...
class Controller {
  ...
  async buildInputs(cfg, minerCfg, snapshot) {
    ...
    const reading = classifyRoomReading(snapshot, zone.thermostat, idleMinutes);
