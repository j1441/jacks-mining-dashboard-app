...
function resolveHeatDemand(cfg, nowIso, roomTempC) {
...
class Controller {
  ...
  async buildInputs(cfg, minerCfg, snapshot) {
    ...
    const alt = ((cfg.heating || {}).alt) || {};
    const roomTempC = estimateRoomTempC(snapshot, (cfg.heating || {}).thermostat);
    this.roomTempC = roomTempC; // cached for snapshotForApi
    const heat = {
      demandKW: resolveHeatDemand(cfg, now, roomTempC),
      altType: alt.type || 'none',
      altPricePerKWh: altHeatPrice(cfg, householdPrice),
      roomTempC,
    };
    if ((cfg.heating || {}).demandSource === 'thermostat' && snapshot.online && roomTempC === null) {
  ...
  currentEconomics(snapshot, cfg) {
    ...
    const alt = ((cfg.heating || {}).alt) || {};
    const heat = {
      demandKW: resolveHeatDemand(cfg, nowIso(), estimateRoomTempC(s, (cfg.heating || {}).thermostat)),
      altType: alt.type || 'none',
      altPricePerKWh: altHeatPrice(cfg, householdPrice),
    };
    let sc;
  ...
  snapshotForApi() {
    ...
    return {
      ...
      roomTempC: estimateRoomTempC(s, ((cfg.heating || {}).thermostat)),
      hashrate: s.hashrate,
      pool: {
...
module.exports = { Controller, resolveHeatDemand, altHeatPrice, estimateRoomTempC, UserActionError };
