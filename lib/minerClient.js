...
// GetHashboardsResponse → [{id, enabled, hashing, boardTempC, chipTempC,
// inletTempC, outletTempC, hashrateThs}]. Inlet is the air entering the board —
// with fans pulling room air it is the closest sensor to ambient temperature.
// It does NOT keep reporting while the miner is paused: measured live 2026-07-29,
// a paused S19j Pro returns lowest_inlet_temp and highest_outlet_temp null on
// every board while board_temp and chip_temp keep updating (a disabled board
// returns null for all four). controller.ambientProxyTempsC handles the fallback.
function parseHashboards(raw) {
  ...
  return list.map((hb) => {
    ...
    return {
      ...
      inletTempC: tempC(hb.lowest_inlet_temp),
      outletTempC: tempC(hb.highest_outlet_temp),
      hashrateThs,
    };
  });
}
...
class MinerClient {
  ...
  async setPowerTarget(watts) {
    ...
    // Verify: SAVE_AND_APPLY reloads the miner config asynchronously, so an
    // immediate GetTunerState can still serve the old target (observed live).
    // Poll for convergence; the SetPowerTarget response already echoed the
    // applied value, so a slow read-back is a warning, not a failure.
    let readTargetW = null;
    for (let i = 0; i < 3; i++) {
      readTargetW = parseTunerState(await this._call('perf', 'GetTunerState', {})).targetW;
      if (readTargetW === applied) break;
      await this._sleep(2000);
    }
    if (readTargetW !== applied) {
      log(`${this.id}: power target read-back still ${readTargetW} W after apply of ${applied} W (miner applying slowly or DPS active)`);
    }
    return { targetW: applied, clamped, readBackW: readTargetW };
  }
  ...
  async setBoards(enableIds = [], disableIds = []) {
    // Verify against the RPC responses' enable-state: SAVE_AND_APPLY persists the
    // config immediately but the runtime board state (GetHashboards) follows only
    // after the hashchain restarts — observed live to lag by minutes.
    const verifyResponse = (res, ids, want) => {
      const byId = new Map(((res && res.hashboards) || []).map((h) => [String(h.id), !!h.is_enabled]));
      for (const id of ids) {
        const got = byId.has(String(id)) ? byId.get(String(id)) : 'missing';
        if (got !== want) throw verifyError(`board ${id} enabled`, want, got);
      }
    };
    if (enableIds.length) {
      const res = await this._call('miner', 'EnableHashboards', {
        save_action: SAVE_AND_APPLY,
        hashboard_ids: enableIds,
      });
      verifyResponse(res, enableIds, true);
    }
    if (disableIds.length) {
      const res = await this._call('miner', 'DisableHashboards', {
        save_action: SAVE_AND_APPLY,
        hashboard_ids: disableIds,
      });
      verifyResponse(res, disableIds, false);
    }
    // Best-effort runtime convergence check — informational only; the controller's
    // TUNING hold and dead-board detection own the long tail of board bring-up.
    let boards = [];
    for (let i = 0; i < 3; i++) {
      boards = parseHashboards(await this._call('miner', 'GetHashboards', {}));
      const byId = new Map(boards.map((b) => [b.id, b.enabled]));
      const converged = enableIds.every((id) => byId.get(String(id)) === true)
        && disableIds.every((id) => byId.get(String(id)) === false);
      if (converged) return { boards, converged: true };
      await this._sleep(2000);
    }
    log(`${this.id}: board config applied (verified via RPC response); runtime state still converging`);
    return { boards, converged: false };
  }
  ...
  async setCoolingMode(cfg) {
    ...
    // Verify the temperature values actually took, not just the mode name — a
    // value change within the same mode would otherwise pass silently.
    const wantTemps = mode === 'manual'
      ? { hotC: request.manual.hot_temperature.degree_c, dangerousC: request.manual.dangerous_temperature.degree_c }
      : { targetC: request[mode].target_temperature.degree_c,
          hotC: request[mode].hot_temperature.degree_c,
          dangerousC: request[mode].dangerous_temperature.degree_c };
    for (const [k, want] of Object.entries(wantTemps)) {
      if (conf.cooling[k] != null && conf.cooling[k] !== want) {
        throw verifyError(`cooling ${k}`, want, conf.cooling[k]);
      }
    }
