...
class History {
  ...
  async minerKWhThisMonth(id) {
    // Called every controller tick — cache for 60 s instead of re-parsing the
    // whole month's NDJSON each time (grows to tens of MB late in the month).
    const cached = this._kwhCache && this._kwhCache[id];
    ...
    const value = wh / 1000;
    this._kwhCache = this._kwhCache || {};
    this._kwhCache[id] = { value, at: Date.now() };
    return value;
  }
