...
async function buildStateSnapshot({ configStore, controllers, market, history, version, tempSensors }) {
  ...
  // One summary per heating zone. Zone room temp = coolest estimate across the
  // zone's online miners (conservative — a colder reading can only increase
  // heat demand, never suppress it).
  const zoneList = (Array.isArray((cfg.heating || {}).zones) && cfg.heating.zones.length)
    ? cfg.heating.zones
    : [{ id: 'home', name: 'Home', ...(cfg.heating || {}) }]; // legacy config without zones
  const zones = zoneList.map((z) => {
    const memberIds = (cfg.miners || [])
      .filter((m) => (m.zoneId || zoneList[0].id) === z.id)
      .map((m) => m.id);
    const temps = (controllers || [])
      .filter((c) => memberIds.includes(c.id))
      // controllers publish an effective (reliability-filtered, held) reading;
      // fall back to a raw estimate only for controllers that never ticked
      .map((c) => (c.roomTempC !== undefined ? c.roomTempC : estimateRoomTempC(c.lastSnapshot, z.thermostat)))
      .filter((t) => t !== null && t !== undefined);
    const inletTempC = temps.length ? Math.min(...temps) : null;
    // A fresh external sensor reading supersedes the inlet estimate — same rule
    // as controller.zoneRoomTemp, kept in step so the UI and the control loop
    // never disagree about what the room temperature is.
    const sensorReading = tempSensors ? tempSensors.reading(z.id) : null;
    const roomTempC = sensorReading ? sensorReading.tempC : inletTempC;
    return {
      id: z.id,
      name: z.name,
      demandSource: z.demandSource || 'off',
      roomTempC,
      roomTempSource: sensorReading ? 'sensor' : (inletTempC === null ? 'none' : 'inlet'),
      inletTempC,
      humidityPct: sensorReading ? sensorReading.humidityPct : null,
      tempSensor: tempSensors
        ? (tempSensors.statuses().find((s) => s.zoneId === z.id) || null)
        : null,
      demandKW: resolveHeatDemand(z, ts, roomTempC, tz),
      altType: (z.alt || {}).type || 'none',
      altPricePerKWh: altHeatPrice(z.alt, householdPrice),
      thermostat: z.thermostat
        ? { targetC: z.thermostat.targetC, bandC: z.thermostat.bandC, maxKW: z.thermostat.maxKW }
        : null,
      miners: memberIds,
    };
  });
...
function createApi({ configStore, controllers, market, history, alerts, version, tempSensors }) {
  const router = express.Router();
  router.use(express.json({ limit: '1mb' }));
  const ctx = { configStore, controllers, market, history, version, tempSensors };
