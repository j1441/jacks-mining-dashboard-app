...
const STROMSTOTTE_THRESHOLD_NOK = 0.9375;  // kr/kWh, applied to the inc-VAT spot (v1 semantics)
...
function computeMarginalPrice({ dateIso, spotExVatNok, electricity, monthMinerKWh = 0 }) {
  ...
  if (electricity.priceMode === 'norgespris') {
    ...
    // strømstøtte: v1 semantics — threshold applied to the inc-VAT spot
    const excess = spotIncVat - STROMSTOTTE_THRESHOLD_NOK;
...
class Market {
  ...
  _compute(dateIso, spotExVat) {
    return computeMarginalPrice({
      ...
      electricity: this._electricity(),
      monthMinerKWh: this._monthMinerKWh,
    });
  }
