...
class Market {
  ...
  fallbackPrice(monthMinerKWh) {
    ...
    if (!past.length) return null;
    // Pessimistic spot-driven base from the trailing window, but the grid fee is
    // deterministic from the clock (never missing), so use the CURRENT hour's fee
    // rather than whatever the trailing hours happened to carry (e.g. a weekend
    // window would otherwise under-fee a weekday-daytime current hour).
    const maxBase = Math.max(...past.map((p) => {
      const r = this._compute(new Date(p.startMs).toISOString(), p.spotExVat);
      return r.marginalPrice - r.components.gridFee;
    }));
    return maxBase + gridFeeAt(nowIso, this._electricity());
  }
